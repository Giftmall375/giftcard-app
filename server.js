...
// ─── JSON file persistence (no native modules needed) ─────────────────────────
const DATA_DIR = process.env.DATA_DIR || __dirname;
const DATA_FILE = path.join(DATA_DIR, 'giftcards.json');

function loadData() {
  try {
    if (fs.existsSync(DATA_FILE)) return JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
  } catch(e) { console.warn('Could not read data file:', e.message); }
  return {
    cards: [],
...
if (data.cards.length === 0) {
  ...
  ].forEach(s => data.cards.push({ ...s, id: data.nextCardId++, created_at: new Date().toISOString() }));
  saveData();
}

function fmtNum(raw) { return raw.replace(/\D/g,'').replace(/(.{4})/g,'$1 ').trim(); }
function findCard(id) { return data.cards.find(c => c.id === parseInt(id)); }
...
app.post('/api/admin/cards', (req, res) => {
  ...
  const card = { id: data.nextCardId++, num: clean, holder: holder||'', balance: parseFloat(balance)||0, expiry, pin: pin||'', status: status||'pending', created_at: new Date().toISOString() };
  data.cards.push(card);
  saveData();
...
app.patch('/api/admin/cards/:id', (req, res) => {
  ...
  Object.assign(card, { num: clean, holder: holder??card.holder, balance: balance!==undefined?parseFloat(balance):card.balance, expiry: expiry??card.expiry, pin: pin??card.pin, status: status??card.status });
  saveData();
...
app.listen(PORT, () => console.log(`✅ Running on port ${PORT} | Data: ${DATA_FILE}`));
